...
app.get('/scoreDisplay/:tName/:match', (req,res) => {
    Tournament.findOne({tournamentName: req.params.tName}, (err, data) => {
        res.render('scoreDisplay', {tournamentName: req.params.tName, listItems: data.matches[req.params.match], index:req.params.match,})
    })
})
...
app.get('/userScoreDisplay/:tName/:match', (req,res) => {
    Tournament.findOne({tournamentName: req.params.tName}, (err, data) => {
        res.render('userScoreDisplay', {tournamentName: req.params.tName, listItems: data.matches[req.params.match]})
    })
})

app.get('/userLeaderboard/:tName', async (req,res) => {
    let data = await Tournament.findOne({tournamentName: req.params.tName})
    ...
    res.render('userLeaderboard', {listItems: data, tournamentName: req.params.tName})
})

app.get('/leaderboard/:tName', async (req,res) => {
    let data = await Tournament.findOne({tournamentName: req.params.tName})
    data.teams.sort(function(a,b) {
        if(a.matchesWon < b.matchesWon) {
            return 1
    ...
    res.render('leaderboard', {listItems: data, tournamentName: req.params.tName})
})

app.get('/declareWinner/:tName/:match', (req,res) => {
    Tournament.findOne({tournamentName: req.params.tName}, (err,data) => {
        res.render('declarewinner', {tournamentName: req.params.tName, listItems: data.matches[req.params.match], index:req.params.match})
    })
})
...
app.post('/declareWinner', async(req,res) => {
    let data = await Tournament.findOne({tournamentName: req.body.tournamentName})
    let match = parseInt(req.body.match)
    let matchWinner = req.body.winner
    data.matches[match].winner = matchWinner

    console.log(req.body)
    console.log(data)
    for(let i=0; i<data.teams.length; i++) {
        if(data.teams[i].teamName===matchWinner) {
            data.teams[i].matchesWon++;
        }
    }
